...
from schemas import GenerationSample, ObjectTrack
...
class InstanceConsistency(BaseModule):
    name = "instance_consistency"

    def __init__(self, config: dict[str, Any] | None = None) -> None:
        super().__init__(config=config)
        self.instance_tracks_key = str(self.config.get("instance_tracks_key", "instance_tracks"))
    ...
    def _evaluate_sample(self, sample: GenerationSample) -> dict[str, Any]:
        ...
        if not view_scores:
            ...
            if not tracks:
                raise _SkipSample("No usable prepared tracks found for expected views or legacy fallback.")
            view_score = self._score_view_tracks(sample, tracks)
            if is_finite_number(view_score):
                view_scores.append(float(view_score))
    ...
    def _collect_view_tracks(self, metadata: dict[str, Any], view: str) -> list[dict[str, Any]]:
    ...
    def _merge_view_mapping(self, tracks: list[dict[str, Any]], mapping: Any, view: str, field_name: str) -> None:
    ...
    def _merge_track_mapping(self, tracks: list[dict[str, Any]], mapping: dict[str, Any], field_name: str) -> None:
    ...
    def _extract_confidences(self, value: Any) -> list[float]:
        ...
        if isinstance(value, dict):
            if "confidence" in value and isinstance(value["confidence"], (int, float)):
                ...
                numeric_values = [float(raw_score) for raw_score in value.values() if isinstance(raw_score, (int, float))]
                if numeric_values:
    ...
    def _normalize_boxes(self, raw_boxes: Any) -> list[dict[str, Any]]:
        ...
        for item in raw_boxes:
            ...
            if bbox_array.size != 4:
                continue
            normalized.append({"frame_index": int(frame_index), "bbox": bbox_array.tolist()})
