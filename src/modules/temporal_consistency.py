...
class TemporalConsistency(BaseModule):
    ...
    def _evaluate_sample(self, sample: Any) -> dict[str, Any]:
        ...
        if not view_scores:
            return skipped_result(sample_id, "No expected camera view produced a valid temporal consistency score.")
    ...
    def _ensure_clip(self) -> str | None:
        ...
        try:
            model, preprocess = clip.load(str(weight_path), device=self.device, jit=False)
            model.eval()
