...
class VideoIntegrityMetric:
    ...
    def evaluate(self, samples: list[Any]) -> dict[str, Any]:
        ...
        pass_rate = (
            safe_div(valid_sample_count, evaluated_count) if evaluated_count > 0 else None
        )
    ...
    def _evaluate_sample(self, sample: Any) -> dict[str, Any]:
        ...
        if len(existing_views) != len(EXPECTED_CAMERA_VIEWS):
            missing_views = [
                view for view in EXPECTED_CAMERA_VIEWS if view not in existing_views
            ]
        ...
        if len(readable_views) != len(EXPECTED_CAMERA_VIEWS):
            unreadable_views = [
                view for view in EXPECTED_CAMERA_VIEWS if view not in readable_views
            ]
            failed_checks.append(
                {
                    "check": "readability",
