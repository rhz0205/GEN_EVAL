...
class TemporalConsistency:
    ...
    def evaluate(self, samples: list[GenerationSample]) -> dict[str, Any]:
        ...
        if not paired_samples:
            return self._result(
                ...
                status="skipped",
                reason="Temporal consistency requires samples with both generated_video and reference_video.",
            )
        ...
        if not video_results:
            ...
            reason = (
                "All paired samples failed during temporal consistency evaluation."
                if failed_samples
    ...
    def _get_runtime(self) -> tuple[_RuntimeBundle | None, str | None]:
        ...
        clip_source = str(weights_path) if weights_path else model_name

        try:
            clip_model, _ = clip.load(clip_source, device=device)
        except Exception as exc:
            return None, f"CLIP model could not be loaded during evaluation: {exc}"
    ...
    def _get_video_image_feature(self, video_path: str, runtime: _RuntimeBundle) -> Any:
        ...
        with runtime.torch.no_grad():
            image_features = runtime.clip_model.encode_image(images)
    ...
    def _calculate_acm(self, image_features: Any, runtime: _RuntimeBundle) -> tuple[float, float, int]:
        if len(image_features) < 2:
            raise ValueError("Temporal consistency requires at least 2 frames per video.")
    ...
    def _calculate_tji(self, image_features: Any, runtime: _RuntimeBundle) -> tuple[Any, Any]:
        if len(image_features) < 3:
            raise ValueError("Temporal consistency requires at least 3 frames per video.")
