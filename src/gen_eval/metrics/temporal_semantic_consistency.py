...
class TemporalSemanticConsistency:
    ...
    def evaluate(self, samples: list[GenerationSample]) -> dict[str, Any]:
        ...
        for sample in samples:
            try:
                masks, num_classes, source_details = self._load_sample_masks(sample, runtime)
            ...
            sample_result = {
                ...
                "S_SAC": scores["S_SAC"],
                "S_CDS": scores["S_CDS"],
                "weights": list(scores["weights"]),
                "params": scores["params"],
                "source": source_details,
            }
            evaluated_samples.append(sample_result)
            total_tscs += scores["TSCS"]
            total_lfr += scores["S_LFR"]
            total_sac += scores["S_SAC"]
            total_cds += scores["S_CDS"]

        if not evaluated_samples:
            status = "failed" if failed_samples else "skipped"
            reason = (
                "All samples failed during temporal semantic consistency evaluation."
                if failed_samples
                else (
                    "No prepared semantic data was found. "
                    f"Provide metadata['{self.semantic_masks_key}'], "
                    f"metadata['{self.segmentation_frames_key}'], or "
                    f"metadata['{self.segmentation_video_key}']."
                )
            )
            return self._result(
                score=None,
        ...
        average_results = {
            "TSCS": total_tscs / num_samples,
            "S_LFR": total_lfr / num_samples,
            "S_SAC": total_sac / num_samples,
            "S_CDS": total_cds / num_samples,
        }
        return self._result(
            score=average_results["TSCS"],
    ...
    def _load_sample_masks(
        ...
        if self.semantic_masks_key in metadata:
            masks = self._load_semantic_masks(metadata[self.semantic_masks_key])
            num_classes = self._resolve_num_classes(metadata, masks)
            return masks, num_classes, {"kind": self.semantic_masks_key}

        if self.segmentation_frames_key in metadata:
            video_rgb = self._load_segmentation_frames(metadata[self.segmentation_frames_key], runtime)
            palette = self._load_palette(metadata.get(self.palette_key))
            masks, num_classes, used_palette = self._labels_from_video_colormap(
                video_rgb, runtime, palette=palette
            )
            return masks, num_classes, {
                "kind": self.segmentation_frames_key,
                "palette_size": int(used_palette.shape[0]),
            }

        if self.segmentation_video_key in metadata:
            video_rgb = self._load_segmentation_video(metadata[self.segmentation_video_key], runtime)
            palette = self._load_palette(metadata.get(self.palette_key))
            masks, num_classes, used_palette = self._labels_from_video_colormap(
                video_rgb, runtime, palette=palette
            )
            return masks, num_classes, {
                "kind": self.segmentation_video_key,
                "palette_size": int(used_palette.shape[0]),
            }

        raise _SkipSample(
            "Missing prepared semantic data. "
            f"Expected metadata key '{self.semantic_masks_key}', "
            f"'{self.segmentation_frames_key}', or '{self.segmentation_video_key}'."
        )
    ...
    def _load_segmentation_frames(self, raw_value: Any, runtime: dict[str, Any]) -> np.ndarray:
    ...
    def _load_segmentation_video(self, raw_value: Any, runtime: dict[str, Any]) -> np.ndarray:
    ...
    def _resolve_num_classes(self, metadata: dict[str, Any], masks: np.ndarray) -> int:
    ...
    def _labels_from_video_colormap(
        ...
        if isinstance(palette, dict):
            ...
            labels = self._map_pixels_to_palette(
                pixels, palette_array, ckd_tree, approximate=self.approximate_palette
            )
        ...
        if self.ignore_color is not None:
            ...
            match_idx = np.where(
                np.all(palette_array == np.asarray(self.ignore_color, dtype=np.uint8), axis=1)
            )[0]
            if match_idx.size > 0:
                labels[ignore_mask] = int(match_idx[0])

        masks = labels.reshape(video_rgb.shape[0], video_rgb.shape[1], video_rgb.shape[2]).astype(
            np.int32
        )
        return masks, num_classes, palette_array

    def _build_palette_from_video(
        self, video_rgb: np.ndarray, max_colors: int | None = None
    ) -> np.ndarray:
    ...
    def _compute_lfr_interior(
        self, masks: np.ndarray, num_classes: int, runtime: dict[str, Any], erosion_k: int
    ) -> float:
        if masks.shape[0] < 2:
            return 1.0
    ...
    def _class_components(
        self, mask: np.ndarray, class_id: int, runtime: dict[str, Any]
    ...
    def _compute_sac(
        self, masks: np.ndarray, num_classes: int, runtime: dict[str, Any], min_iou: float
    ) -> float:
        if masks.shape[0] < 2:
            return 1.0
    ...
    def _compute_cds(self, masks: np.ndarray, num_classes: int, runtime: dict[str, Any]) -> float:
        if masks.shape[0] < 2:
            return 1.0
    ...
    def _tscs_score(
        ...
        *,
        ...
        s_lfr = self._compute_lfr_interior(masks, num_classes, runtime, erosion_k=erosion_k)
