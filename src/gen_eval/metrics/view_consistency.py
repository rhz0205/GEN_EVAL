...
from .video_integrity import EXPECTED_CAMERA_VIEWS, get_cv2, inspect_video
...
class ViewConsistencyMetric:
    ...
    def _evaluate_sample(self, sample: Any) -> dict[str, Any]:
        ...
        if not self._loftr_is_configured():
            return self._skipped_sample(
                sample_id,
                "LoFTR repo_path/weight_path is not configured.",
            )
        ...
        video_infos = {
            view: inspect_video(normalized_videos[view]) for view in EXPECTED_CAMERA_VIEWS
        }
        unreadable_views = [
            view
            for view, info in video_infos.items()
            if not bool(info.get("readable"))
        ]
        if unreadable_views:
            return self._skipped_sample(
                sample_id,
        ...
        for cam_a, cam_b, side_a, side_b in ADJACENT_CAMERA_PAIRS:
            ...
            if pair_score is None:
                return self._skipped_sample(
                    sample_id,
                    f"Unable to evaluate adjacent pair {cam_a}|{cam_b}.",
                )
