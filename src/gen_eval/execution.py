...
from gen_eval.evaluator import Evaluator, iter_enabled_metrics, normalize_metric_result, run_metric
...
def _run_ray(config: dict[str, Any]) -> dict[str, Any]:
    ...
    num_workers = _coerce_positive_int(runtime.get("num_workers"))
    ...
    def evaluate_sample(
        ...
        register_builtin_metrics()
        sample = GenerationSample.from_dict(sample_payload)
        return run_metric(metric_name, metric_config, [sample])

    results = []
    max_in_flight = num_workers or max(1, len(sample_payloads))
    ...
    for metric_name, metric_config in iter_enabled_metrics(config.get("metrics", {})):
        ...
        pending = [
            evaluate_sample.remote(metric_name, metric_config, sample_payload)
            for sample_payload in sample_payloads
        ]
        partials = _ray_gather_limited(ray, pending, max_in_flight)
        results.append(_merge_metric_results(metric_name, partials, len(sample_payloads)))
...
def _ray_gather_limited(ray_module: Any, refs: list[Any], max_in_flight: int) -> list[dict[str, Any]]:
...
def _merge_metric_results(
    ...
    averaged_detail_fields: dict[str, list[tuple[dict[str, Any], int]]] = defaultdict(list)
    ...
    for partial in partial_results:
        ...
        weight = int(num_samples) if isinstance(num_samples, int) and num_samples > 0 else 0
    ...
    if valid_scores:
        ...
        reason = reasons[0] if reasons else f"No sample produced a valid {metric_name} score."
        num_valid_samples = 0
...
def _merge_numeric_mappings(weighted_values: list[tuple[dict[str, Any], int]]) -> dict[str, float]:
