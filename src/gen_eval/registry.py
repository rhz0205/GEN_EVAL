...
class MetricRegistry:
    """Store canonical metric classes and optional aliases."""
    ...
    def register(self, metric_class: type) -> type:
        """Register a metric class by its canonical `name` attribute."""
    ...
    def register_alias(self, alias: str, target: str) -> None:
        """Register a simple alias that resolves to a canonical metric name."""
        if target not in self._metrics:
            raise KeyError(f"Cannot register alias '{alias}' for unknown metric '{target}'.")
        self._aliases[alias] = target

    def get(self, name: str) -> type:
        """Return the metric class for a canonical name or alias."""
    ...
    def items(self) -> Iterable[tuple[str, type]]:
        """Iterate over registered names with resolved metric classes."""
