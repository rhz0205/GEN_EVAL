...
def summarize_metric(metric_result: dict[str, Any]) -> list[str]:
    ...
    lines.extend(summarize_view_data(evaluated_samples))
    lines.extend(summarize_pair_data(evaluated_samples))
    return lines
...
def format_result_summary(
    ...
    for item in metric_results:
        if not isinstance(item, dict):
            continue
        lines.extend(summarize_metric(item))
    return lines


def summarize_result_file(result_path: str | Path) -> list[str]:
    payload = load_result_json(result_path)
    return format_result_summary(payload, result_path)
