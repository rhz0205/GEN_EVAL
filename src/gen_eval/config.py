...
def normalize_runtime_config(runtime: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not isinstance(backend, str):
        raise ValueError(
            "Run config field 'runtime.backend' must be a string when provided."
        )
    ...
    if backend_name == "ray":
        ...
        runtime_config.setdefault("num_gpus_per_task", 0.0)
        runtime_config.setdefault("num_cpus_per_task", 1)
    else:
        runtime_config.setdefault("max_in_flight", 0)
